...
const rows = container
  .selectAll("div.row")
  .data(d3.range(Math.ceil(imageList.length / 5))) // Calculate the number of rows needed
  .enter()
  .append("div")
  .attr("class", "row"); // Add a class to style the rows (optional)
...
rows
  ...
  .attr("id", (d, i) => "image-" + (imageList.indexOf(d) + 1)) // Add the "image-" prefix to the ids
  .style("height", "4.2vw") // Set the height to 4.2% of viewport
  .style("margin-right", "10px") // Add some spacing between images
